...
class Tiles:
    def __init__(self,gridPos,type,position,id = 0,image = ""):
        ...
        if type == TileType.image:
            ...
            self.animationSize = self.Establish_surface_type(self.surface)
            if self.animationSize:
                #self.animation = Animation("GruesomeMapEditor\Images")
    ...
    def Mouse_tile_preview(self,display,grid,mousePos):
        ...
        if self.animationSize:
            ...
            display.blit(self.mouse_preview_surface,grid.GetMainWorldLoc(mousePos))
    ...
    def Establish_surface_type(self,surface:Surface) -> Tuple[int,int]: 
